...
#------------------------------------------------------------------------------
hex_to_str:
	li $t3, 28	# Begin hex_to_str()
hex_to_str_loop:
	bltz $t3, hex_to_str_end	# Shift amount < 0, finished
	srlv $t2, $a0, $t3	# ($a0 >> $t3)
	andi $t2, $t2, 0xf	# Take lower 4 bits: x
	bgeu $t2, 10, hex_to_str_hex	# Judge whether dec digit / hex digit
hex_to_str_dec:
	addiu $t2, $t2, 48	# 0 <= x < 10, convert to dec digit
	j hex_to_str_next
hex_to_str_hex:
	addiu $t2, $t2, 87	# 10 <= x, convert to hex digit
hex_to_str_next:
	sb $t2, 0($a1)	# Step forward
	addiu $a1, $a1, 1
	subiu $t3, $t3, 4
	j hex_to_str_loop
hex_to_str_end:
	...
	sb $t2, 0($a1)
	addiu $a1, $a1, 1	# Write '\0'
	sb $0, 0($a1)
	jr $ra	# End hex_to_str()
