...
inst_needs_relocation:
	srl $a0, $a0, 26	# Begin inst_needs_relocation()
	beq $a0, 2, inst_needs_relocation_true	# Is j?
	beq $a0, 3, inst_needs_relocation_true	# Is jal?
inst_needs_relocation_false:	# Not j / jal
	li $v0, 0	# Return 0
	jr $ra
inst_needs_relocation_true:
	li $v0, 1	# Return 1
	jr $ra	# End inst_needs_relocation()
...
#------------------------------------------------------------------------------
relocate_inst:
	addiu $sp, $sp, -8	# Begin relocate_inst
	sw $s6, 4($sp)	# Make stack
	sw $ra, 0($sp)
	move $s6, $a0	# Save original instruction in #s0
	
	move $a0, $a1	# Call symbol_for_addr(offset, reltbl)
	move $a1, $a3
	jal symbol_for_addr
	beq $v0, $0, relocate_inst_error	# Failed
	
	move $a0, $a2	# Call addr_for_symbol(symtbl, addr)
	move $a1, $v0
	jal addr_for_symbol
	beq $v0, -1, relocate_inst_error	# Failed
relocate_inst_success:
	andi $s6, 0xfc000000	# Re-write instruction
	srl $v0, $v0, 2	# Remove lowerest two zeros
	or $s6, $s6, $v0	# Write the absolute address
	move $v0, $s6	# Return the relocated instruction
