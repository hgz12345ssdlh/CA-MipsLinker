...
write_machine_code:
	...
	addiu $sp, $sp, -32
	sw $t5, 28($sp)
				# Offset 24 used for storing Byte offset
	sw $s6, 12($sp)
	sw $s7, 8($sp)
						# offset 4 is used later...
	sw $ra, 0($sp)
	# We'll save the arguments since we are making function calls.
	sw $a0, 16($sp)		# sp+16 = output file ptr
	sw $a1, 20($sp)     # sp+20 = input file ptr
	move $s6, $a2			# $s6 = symbol table
	move $s7, $a3			# $s7 = relocation table
...
write_machine_code_next_inst:
	...
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $v1	# Call parse_int() on the instruction string
	li $a1, 16	# with base 16
	jal parse_int
	move $t5, $v0	# 32-bits instruction stored in $t5
	
	# 4. Check if the instruction needs relocation. If it does not, branch to
	# the label write_machine_code_to_file:
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $t5	# Call inst_needs_relocation() on instruction
	jal inst_needs_relocation
	beq $v0, $0, write_machine_code_to_file	# No need to relocate
	...
	# YOUR_INSTRUCTIONS_HERE
	move $a0, $t5	# Call relocate_inst() on instruction
	lw $a1, 24($sp) # Extract addr from 24($sp)
	move $a2, $s6
	move $a3, $s7
	jal relocate_inst
	blt $v0, $0, write_machine_code_error	# Relocation failed, jump to error
	move $t5, $v0	# Relocated instruction stored back into $t5

write_machine_code_to_file:
	# 6. Write the instruction into a string buffer via hex_to_str():
	# YOUR_INSTRUCTIONS_HERE 
	move $a0, $t5	# Call hex_to_str() on instruciton
	la $a1, hex_buffer	# Result stored in hex_buffer
	jal hex_to_str
	
	# 7. Increment the byte offset by the appropriate amount:
	# YOUR_INSTRUCTIONS_HERE
	lw $t4, 24($sp)
	addiu $t4, $t4, 4	# Increment offset by 4
	sw $t4, 24($sp)
	...
	lw $t8, 4($sp)
	bne $t8, $zero, write_machine_code_next_inst
