...
addr_for_symbol:
	...
	sw $s7, 4($sp)
	sw $ra, 0($sp)
	move $s6, $a0	# Save current node in $s6
	move $s7, $a1	# Save name in $s7
addr_for_symbol_loop:
	beq $s6, $0, addr_not_found	# Reaches end of list, return -1
	lw $a0, 4($s6)	# Read name of current node
	move $a1, $s7	# Call streq()
	jal streq
	beq $v0, $0, addr_found	# $a0 == $a1, Found
	lw $s6, 8($s6)	# Else, go to next node
	j addr_for_symbol_loop
addr_found:
	lw $v0, 0($s6)	# Return address of current node
...
#------------------------------------------------------------------------------
add_to_list:	
	addiu $sp, $sp, -12	# Begin add_to_list()
	sw $s6, 8($sp)	# Make stack
	sw $s7, 4($sp)
	sw $ra, 0($sp)
	move $s6, $a0	# Save pointer to list in $s6
	
	jal new_node	# Create a new node
	move $s7, $v0	# Pointer to new node stored in $s7
	
	sw $s6, 8($s7)	# new_node->next = current_front
	sw $a1, 0($s7)	# new_node->addr = addr
	move $a0, $a2	# Call copy_of_str() on name
	jal copy_of_str
	sw $v0, 4($s7)	# new_node->name = copy of name
	
	move $v0, $s7	# Return pointer to new node
	lw $ra, 0($sp)	# Recycle stack
	lw $s7, 4($sp)
	lw $s6, 8($sp)
	addiu $sp, $sp, 12
